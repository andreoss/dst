...
static void when_differs_should_be_unsynced(void **state) {
  Report *rep = report_new();
  report_pend(rep, "???");
  assert_true(report_is_unsynced(rep));
}
static void when_differs_should_be_synced(void **state) {
  Report *rep = report_new();
  report_pend(rep, "???");
  report_pop(rep);
...
static void becomes_synced_when_popped(void **state) {
  Report *rep = report_new();
